...
import { KeyRound, Trash2, ListTodo } from "lucide-react";
import { Container, Wrapper } from "@/components/ui/primitives";
import { BackToHome } from "@/components/ui/buttons";
import { useToast } from "@/providers/toast-provider";

type HistoryItem = { id: string; name: string; keywordCount: number; createdAt: string };

export function CollectionHistoryList({ items: initialItems }: { items: HistoryItem[] }) {
  ...
  return (
    <Container>
      ...
      <h1 className="text-[#171717] text-[28px] font-bold mb-6">My Saved Keyword Collections</h1>

      {items.length === 0 ? (
        <Wrapper className="bg-white rounded-2xl border border-black/5 shadow-6xl p-10 text-center mb-16">
          <ListTodo className="w-10 h-10 mx-auto mb-3 text-black/20" aria-hidden="true" />
          <p className="text-[#475569] mb-4">You haven&apos;t saved any keyword collections yet.</p>
          <Link
            href="/keyword-planner"
        ...
        <Wrapper className="grid gap-3 mb-16">
          {items.map((item) => (
            ...
            >
              <Link href={`/keyword-planner/collections/${item.id}`} className="flex-1 min-w-0">
                <span className="inline-block px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-600 text-xs font-medium mb-1.5">
                  {item.keywordCount} keyword{item.keywordCount === 1 ? "" : "s"}
                </span>
                <p className="text-[#171717] font-semibold truncate">{item.name}</p>
